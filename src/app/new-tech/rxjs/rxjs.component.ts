...
export class RxjsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //hot publish connect
    //hot call once
    let obsHot1 = Observable.create((observer: { next: (arg0: number) => void; }) => {let now = Date.now();console.log("hot: " + now);observer.next(now)}).pipe(publish()) as ConnectableObservable<any>
    obsHot1.subscribe(v => console.log("1st subscriber hot: " + v));
    obsHot1.subscribe(v => console.log("2nd subscriber hot: " + v));
    obsHot1.connect()

        // let obsHot2 = Observable.create(((observe: { next: (arg0: number) => void; })r: { next: (arg0: number) => void; }) => {let now = Date.now();console.log("hot2: " + now);observer.next(now)}).pipe(shareReplay()) as ConnectableObservable<any>
...
15:14:49.781 rxjs.component.ts:26 2nd subscriber cold: 1551856489780
    ...
    let obsCold1 = Observable.create((observer: { next: (arg0: number) => void; }) => {let now = Date.now();console.log("cold: " + now);observer.next(now)}) as Observable<any>
...
15:00:59.136 rxjs.component.ts:39 1st subscriber obsWarm:4
  ...
  contacts2: Observable<Array<any>> | undefined;
  contacts1: Observable<Array<any>> | undefined;
  ...
  rxjsTest() {
    ...
    var observerA = {
      next: (value: string) => console.log('Observer A get value: ' + value),
      error: (error: string) => console.log('Observer A error: ' + error),
      complete: () => console.log('Observer A complete!')
    };

    var observerB = {
      next: (value: string) => console.log('Observer B get value: ' + value),
      error: (error: string) => console.log('Observer B error: ' + error),
